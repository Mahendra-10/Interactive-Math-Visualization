...
void Image::set_pixels(int row, int col, Color newColor) {
    
    if (row >= 0 && row < m_height && col >= 0 && col < m_width) { //bounds checking
        m_pixels[row * m_width + col] = newColor;
    } else {
      throw PlotException("Out of bounds.");
